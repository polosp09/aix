...
from aix.models import EntityModel, ItemFetThroughModel, LazyLoader
...
class EquipmentFetModelManager(models.Manager):
    ...
    def for_fets(self, entity_slug, user_model):
        qs = self.get_queryset()
        if isinstance(entity_slug, EntityModel):
            qs = qs.filter(entity=entity_slug)
        elif isinstance(entity_slug, str):
...
class EquipmentFetModel(CreateUpdateMixIn, MarkdownNotesMixIn):
    FET_STATUS_DRAFT = 'draft'
    FET_STATUS_REVIEW = 'in_review'
    FET_STATUS_APPROVED = 'approved'
    FET_STATUS_FULFILLED = 'fulfilled'
    FET_STATUS_PAID = 'paid'
    FET_STATUS_VOID = 'void'
    FET_STATUS_CANCELED = 'canceled'

    FET_STATUS = [
        (FET_STATUS_DRAFT, _('Draft')),
        (FET_STATUS_REVIEW, _('In Review')),
        (FET_STATUS_APPROVED, _('Approved')),
        (FET_STATUS_FULFILLED, _('fulfilled')),
        (FET_STATUS_PAID, _('Paid')),
        (FET_STATUS_CANCELED, _('Canceled')),
        (FET_STATUS_VOID, _('Void'))
    ]

    uuid = models.UUIDField(default=uuid4, editable=False, primary_key=True)    
    code = models.CharField(blank=False, null=False, max_length=50)
    fet_date = models.DateField(blank=True, null=True)
    fet_status = models.CharField(max_length=15,
                                      choices=FET_STATUS,
                                      blank=True,
                                      null=True,
                                      default=FET_STATUS_DRAFT,
    ...
    entity = models.ForeignKey('aix.EntityModel',
                               ...
                               verbose_name=_('EquipmentFet Entity'))
    ...
    fet_items = models.ManyToManyField('aix.EquipmentModel',
                                      through='aix.ItemFetThroughModel',
                                      through_fields=('fet_model', 'equipment_model'),
                                      verbose_name=_('FET Items'))

    objects = EquipmentFetModelManager()

    class Meta:
        verbose_name = _('EquipmentFet')
        indexes = [
            models.Index(fields=['created']),
            models.Index(fields=['updated']),
        ]
        unique_together = [
            ('entity', 'fet_date', 'location', 'work_order')
        ]

    def __str__(self):
        return f'{self.fet_date} - {self.fet_date} - {self.work_order}'
    ...
    def configure(self,
        ...
        if fet_date:
            self.fet_date = fet_date
        self.entity = entity_model
        return self

    # State Update...
    def get_fet_item_data(self, queryset: QuerySet = None) -> Tuple:
        if not queryset:
            # pylint: disable=no-member
            queryset = self.itemfetthroughmodel_set.filter(fet_model_id=self.fet_model).select_related('fet_model')

        return queryset, queryset.aggregate(
            total_items=Count('uuid')
        )

    def get_fet_staff_data(self, queryset: QuerySet = None) -> Tuple:
        if not queryset:
            # pylint: disable=no-member
            # queryset = EquipmentModel.objects.all().select_related('entity')
            queryset = None
...
# State...
    def is_draft(self) -> bool:
        return self.fet_status == self.FET_STATUS_DRAFT

    def is_review(self) -> bool:
        return self.fet_status == self.FET_STATUS_REVIEW

    def is_approved(self) -> bool:
        return self.fet_status == self.FET_STATUS_APPROVED

    def is_fulfilled(self) -> bool:
        return self.fet_status == self.FET_STATUS_FULFILLED

    def is_canceled(self) -> bool:
        return self.fet_status == self.FET_STATUS_CANCELED

    def is_void(self) -> bool:
        return self.fet_status == self.FET_STATUS_VOID
    ...
    def mark_as_draft(self, commit: bool = False, **kwargs):
        ...
        self.fet_status = self.FET_STATUS_DRAFT
        self.clean()
        if commit:
            self.save(update_fields=[
                'fet_status',
    ...
    def mark_as_review(self, date_review: date = None, commit: bool = False, **kwargs):
        ...
        self.fet_status = self.FET_STATUS_REVIEW
        self.clean()
        if commit:
            self.save(update_fields=[
                'fet_status',
    ...
    def mark_as_approved(self, commit: bool = False, date_approved: date = None, **kwargs):
        ...
        self.fet_status = self.FET_STATUS_APPROVED
        self.clean()
        if commit:
            self.itemthroughmodel_set.all().update(jcd_item_status=ItemFetThroughModel.STATUS_ORDERED)
            self.save(update_fields=[
                'approved_date',
                'fet_status',
                'updated'
    ...
    def mark_as_canceled(self, commit: bool = False, date_canceled: date = None, **kwargs):
        ...
        self.fet_status = self.FET_STATUS_CANCELED
        self.clean()
        if commit:
            self.save(update_fields=[
                'fet_status',
    ...
    def mark_as_fulfilled(self,
                          ...
                          jcd_items: Union[QuerySet, List[ItemFetThroughModel]] = None,
                          commit=False,
                          **kwargs):
        ...
        self.fet_status = self.FET_STATUS_FULFILLED
        self.clean()

        if commit:
            jcd_items.update(jcd_item_status=ItemFetThroughModel.STATUS_RECEIVED)
            self.save(update_fields=[
                'fulfillment_date',
                'fet_status',
                'updated'
    ...
    def mark_as_void(self,
        ...
        self.fet_status = self.FET_STATUS_VOID
        self.clean()
