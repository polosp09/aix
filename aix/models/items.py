...
class ItemModelManager(models.Manager):
    ...
    def for_po_fuel(self, entity_slug: str, user_model):
        qs = self.for_entity_active(entity_slug=entity_slug, user_model=user_model)
        qs = qs.filter(
                    Q(is_product_or_service=False, for_inventory=False) |
                    Q(for_inventory=True))
        qs = qs.filter(Q(item_type='F'))
        return qs

    def for_jcd(self, entity_slug: str, user_model):
        qs = self.for_entity_active(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(
            Q(is_product_or_service=True) |
            Q(for_inventory=False)
...
class ItemModelAbstract(CreateUpdateMixIn):
    ...
    uom = models.ForeignKey('aix.UnitOfMeasureModel',
...
class ItemThroughModelQueryset(models.QuerySet):
    ...
    def is_jcd_received(self):
        return self.filter(jcd_item_status=ItemThroughModel.STATUS_RECEIVED)

    def in_jcd_transit(self):
        return self.filter(jcd_item_status=ItemThroughModel.STATUS_IN_TRANSIT)

    def is_jcd_ordered(self):
        return self.filter(jcd_item_status=ItemThroughModel.STATUS_ORDERED)
...
class ItemThroughModelManager(models.Manager):
    ...
    def for_jcd(self, entity_slug, user_model, jcd_pk):
        qs = self.for_entity(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(jcd_model__uuid__exact=jcd_pk)

    def for_jobcard(self, entity_slug, user_model, jcd_pk):
        qs = self.for_entity(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(jcd_model__uuid__exact=jcd_pk)
    ...
    def inventory_received_count(self, entity_slug, user_model):
        qs = self.for_entity(entity_slug=entity_slug, user_model=user_model)
        qs = qs.filter(
            Q(item_model__for_inventory=True) 
        )
    ...
    # jobcard functions
    def inventory_jcd_pipeline(self, entity_slug, user_model):
        qs = self.for_entity(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(
            Q(item_model__for_inventory=True) &
            Q(invoice_model__isnull=False) &
            Q(jcd_item_status__in=[
                ItemThroughModel.STATUS_ORDERED,
                ItemThroughModel.STATUS_IN_TRANSIT,
                ItemThroughModel.STATUS_RECEIVED,
            ])
    ...
    def inventory_jcd_pipeline_aggregate(self, entity_slug: str, user_model):
        qs = self.inventory_pipeline(entity_slug=entity_slug, user_model=user_model)
        return qs.values(
            'item_model__name',
            'item_model__uom__name',
            'jcd_item_status').annotate(
            total_quantity=Sum('quantity'),
            total_value=Sum('total_amount')
        )

    def inventory_jcd_pipeline_ordered(self, entity_slug, user_model):
        qs = self.inventory_pipeline(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(jcd_item_status=ItemThroughModel.STATUS_ORDERED)

    def inventory_jcd_pipeline_intransit(self, entity_slug, user_model):
        qs = self.inventory_pipeline(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(jcd_item_status=ItemThroughModel.STATUS_IN_TRANSIT)

    def inventory_jcd_pipeline_received(self, entity_slug, user_model):
        qs = self.inventory_pipeline(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(jcd_item_status=ItemThroughModel.STATUS_RECEIVED)

    def inventory_jcd_invoiced(self, entity_slug, user_model):
        qs = self.for_entity(entity_slug=entity_slug, user_model=user_model)
        return qs.filter(
            Q(item_model__for_inventory=True) &
            Q(invoice_model__isnull=False)
    ...
    def inventory_jcd_count(self, entity_slug, user_model):
        qs = self.for_entity(entity_slug=entity_slug, user_model=user_model)
        qs = qs.filter(
            Q(item_model__for_inventory=True) &
            (
                # received inventory...
                    (
                            Q(invoice_model__isnull=False) &
                            Q(jcd_model__jcd_status='approved') &
                            Q(jcd_item_status__exact=ItemThroughModel.STATUS_RECEIVED)
                    ) |
...
class ItemThroughModelAbstract(ParentChildMixIn, CreateUpdateMixIn):
    ...
    item_model = models.ForeignKey('aix.ItemModel',
                                   on_delete=models.PROTECT,
                                   verbose_name=_('Item Model'))
    bill_model = models.ForeignKey('aix.BillModel',
                                   on_delete=models.PROTECT,
                                   null=True,
                                   blank=True,
                                   verbose_name=_('Bill Model'))
    invoice_model = models.ForeignKey('aix.InvoiceModel',
                                      on_delete=models.PROTECT,
    ...
    po_model = models.ForeignKey('aix.PurchaseOrderModel',
                                 on_delete=models.PROTECT,
                                 null=True,
                                 blank=True,
    ...
    # Jobcard fields...
    jcd_model = models.ForeignKey('aix.WorkOrderJobcardModel',
                                 on_delete=models.PROTECT,
                                 null=True,
                                 blank=True,
                                 verbose_name=_('Jobcard Model'))
    jcd_item_status = models.CharField(max_length=15,
                                      choices=JCD_ITEM_STATUS,
                                      blank=True,
                                      null=True,
                                      verbose_name=_('JCD Item Status'))
    jcd_quantity = models.FloatField(default=0.0,
                                    verbose_name=_('JCD Quantity'),
                                    help_text=_('Authorized item quantity for purchasing.'),
                                    validators=[MinValueValidator(0)])
    jcd_unit_cost = models.FloatField(default=0.0,
                                     null=True,
                                     blank=True,
                                     verbose_name=_('JCD Unit Cost'),
                                     help_text=_('Jobcard unit cost.'),
                                     validators=[MinValueValidator(0)])
    jcd_total_amount = models.DecimalField(max_digits=20,
                                          default=Decimal('0.00'),
                                          decimal_places=2,
                                          verbose_name=_('Authorized maximum item cost per Jobcard'),
                                          help_text=_('Maximum authorized cost per Jobcard.'),
                                          validators=[MinValueValidator(0)])
    jcd_fuel_type = models.CharField(max_length=15,
                                      choices=JCD_FUEL_TYPES,
                                      blank=True,
                                      null=True,
                                      verbose_name=_('JCD Fuel Type')) 
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['jcd_model', 'item_model']),
            models.Index(fields=['ce_model', 'item_model']),
            models.Index(fields=['po_item_status']),
            models.Index(fields=['jcd_item_status']),
        ]
    ...
    def __str__(self):
        ...
        if self.jcd_model_id:
            jcd_status_display = self.get_jcd_item_status_display()
            return f'JOBCARDx: {self.uuid} | {self.jcd_item_status} | {amount}'
    ...
    def is_jcd_delivered(self):
        return self.jcd_item_status == self.STATUS_RECEIVED

    def is_jcd_canceled(self):
        return self.jcd_item_status == self.STATUS_CANCELED
    ...
    def update_total_amount(self):
        ...
        if self.po_total_amount > 0:
            if total_amount > self.po_total_amount:
                ...
                if diff > MANDO_TRANSACTION_MAX_TOLERANCE:
                    raise ValidationError(
                        f'Difference between PO Amount {self.po_total_amount} and Bill {total_amount} '
                        f'exceeds tolerance of {MANDO_TRANSACTION_MAX_TOLERANCE}')
                self.total_amount = self.po_total_amount
                return

        if self.jcd_total_amount > 0:
            if total_amount > self.jcd_total_amount:
                # checks if difference is within tolerance...
                diff = total_amount - self.jcd_total_amount
                if diff > MANDO_TRANSACTION_MAX_TOLERANCE:
                    raise ValidationError(
                        f'Difference between PO Amount {self.jcd_total_amount} and Bill {total_amount} '
                        f'exceeds tolerance of {MANDO_TRANSACTION_MAX_TOLERANCE}')
                self.total_amount = self.jcd_total_amount
                return
    ...
    def update_jcd_total_amount(self):
        self.jcd_total_amount = round(Decimal.from_float(self.jcd_quantity * self.jcd_unit_cost), 2)
    ...
    def can_create_invoice(self):
        # pylint: disable=no-member
        return self.invoice_model_id is None and self.jcd_item_status in [
            self.STATUS_ORDERED, self.STATUS_IN_TRANSIT, self.STATUS_RECEIVED
        ]

    def can_jcd_create_invoice(self):
        # pylint: disable=no-member
        return self.invoice_model_id is None and self.jcd_item_status in [
            self.STATUS_ORDERED, self.STATUS_IN_TRANSIT, self.STATUS_RECEIVED
        ]
    ...
    def get_jcd_status_css_class(self):
        if self.jcd_item_status == self.STATUS_RECEIVED:
            return ' is-success'
        elif self.jcd_item_status == self.STATUS_CANCELED:
            return ' is-danger'
        elif self.jcd_item_status == self.STATUS_ORDERED:
            return ' is-info'
        return ' is-warning'
    ...
    def clean(self):
        ...
        # pylint: disable=no-member
        if self.jcd_model_id and self.po_model_id:
            if self.quantity > self.jcd_quantity:
                raise ValidationError(f'Invoiced quantity {self.quantity} cannot be greater than '
                                      f'JCD quantity {self.jcd_quantity}')
            if self.total_amount > self.jcd_total_amount:
                raise ValidationError(f'Item amount {self.total_amount} cannot exceed authorized '
                                      f'JCD amount {self.jcd_total_amount}')
        else:
            self.jcd_item_status = None
