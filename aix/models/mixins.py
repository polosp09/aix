...
from aix.io import balance_tx_data, ASSET_CA_CASH, ASSET_CA_PREPAID, LIABILITY_CL_DEFERRED_REVENUE
...
class LedgerWrapperMixIn(models.Model):
    ...
    ledger = models.OneToOneField('aix.LedgerModel',
                                  editable=False,
    ...
    def get_invoice_amount_paid(self, user_model: UserModel):
        amt_paid = 0
        invoice_model: InvoiceModel = self
        # amt_paid_values = self.instances.for_txs(
        ...
        amt_paid = sum(amt.amount for amt in txs_queryset)
        return amt_paid
...
# +++++++++++++++  check and troubleshoot
        item_data_gb = groupby(item_data,
                               key=lambda a: ('item_model__earnings_account__uuid', #a['account_uuid'],
                                              a['entity_unit__uuid'],))
        ...
        # modify to suit amount paid calculations installments
        diff_idx = {
        ...
        if commit:
            ...
            print(je_list)
            print(diff_idx.items())
            accountx = '6fe4fe62-eba4-4a3b-bf5c-a23fdd2c5ae5'
            # account_model: AccountModel = get_object_or_404(uuid__exact=accountx)
            tx_typex = 'debit'
            amountx = 0
            for amt in diff_idx.items():
                amountx = amt[1]
            ...
            txs = [
                {'account': accountx, 'journal_entry': je_list, 'amount': amountx, }
            ]
...
class MarkdownNotesMixIn(models.Model):
    ...
    def notes_html(self):
        ...
        return force_str(self.markdown_notes)
