"""
Contributions to this module:
Miguel Sanda <msanda@arrobalytics.com>
Mando <mandonpolo@gmail.com>
"""

from uuid import uuid4

from django.core.validators import MinValueValidator
from django.db import models
from django.utils.translation import gettext_lazy as _

from aix.models.mixins import SlugNameMixIn, CreateUpdateMixIn, SlugNameMixIn
from django.db.models import Q


class BenefitTypeModelManager(models.Manager):

    def for_entity(self, entity_slug: str, user_model):
        qs = self.get_queryset()
        return qs.filter(
            Q(entity__slug__exact=entity_slug) &
            #Q(active=True) &
            (
                    #Q(entity__admin=user_model) |
                    Q(entity__managers__in=[user_model])
            )
        )


class BenefitTypeModel(CreateUpdateMixIn):
    uuid = models.UUIDField(default=uuid4, editable=False, primary_key=True)
    code = models.CharField(max_length=100, default='NA')
    name = models.CharField(max_length=100)
    polarity = models.IntegerField(default=0)
    value = models.TextField(blank=True, null=True)
    algorithm = models.TextField(blank=True, null=True)
    entity = models.ForeignKey('aix.EntityModel',
                               editable=False,
                               on_delete=models.CASCADE,
                               verbose_name=_('BenefitType Entity'),
                               related_name='benefit_types')
    benefit_calculation = models.ForeignKey('aix.BenefitCalculationModel',
                               on_delete=models.RESTRICT,
                               verbose_name=_('Benefit Calculation Entity'))
    description = models.TextField()
    is_taxed = models.BooleanField(default=True)
    is_active = models.BooleanField(default=False)

    objects = BenefitTypeModelManager()

    class Meta:
        verbose_name = _('BenefitType')
        indexes = [
            models.Index(fields=['created']),
            models.Index(fields=['updated']),
            models.Index(fields=['value']),
        ]
        unique_together = [
            ('entity', 'code')
        ]

    def __str__(self):
        return f'BenefitType: {self.code}'
