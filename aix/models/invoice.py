...
from aix.models import LazyLoader
from aix.models.customer import CustomerModel
from aix.models.entity import EntityModel
from aix.models.mixins import CreateUpdateMixIn, LedgerWrapperMixIn, MarkdownNotesMixIn
...
class InvoiceModelManager(models.Manager):
    ...
    def for_entity_owed(self, entity_slug, user_model, customer):
        qs = self.for_customer(entity_slug=entity_slug, user_model=user_model, customer=customer)
        return qs
    ...
    def for_customer(self, entity_slug, user_model, customer):
        ...
        if isinstance(entity_slug, EntityModel):
            qs = qs.filter(ledger__entity=entity_slug)
        elif isinstance(entity_slug, str):
            qs = qs.filter(ledger__entity__slug__exact=entity_slug)
            
        if isinstance(customer, CustomerModel):
            qs = qs.filter(customer=customer)
        elif isinstance(customer, str):
            qs = qs.filter(customer_id__exact=customer)
        return qs

    def for_work_order(self, entity_slug, user_model, work_order):
        qs = self.get_queryset().filter(
            Q(ledger__entity__admin=user_model) |
            Q(ledger__entity__managers__in=[user_model])
        )
        ...
        if isinstance(work_order, WorkOrderModel):
            qs = qs.filter(work_order=work_order)
        elif isinstance(work_order, str):
            qs = qs.filter(work_order_id__exact=work_order)
        return qs
    ...
    def for_counter(self, entity_slug):
        qs = self.get_queryset().all()
        if isinstance(entity_slug, EntityModel):
            qs = qs.filter(ledger__entity__slug=entity_slug)
        elif isinstance(entity_slug, str):
            qs = qs.filter(ledger__entity__slug__exact=entity_slug)

    def for_entity_invoices(self, entity_slug, user_model):
        qs = self.get_queryset().filter(
            Q(ledger__entity__admin=user_model) |
            Q(ledger__entity__managers__in=[user_model])
        )
    ...
    def for_customer_detail(self, entity_slug, customer=None):
        qs = self.get_queryset().all()
        if isinstance(entity_slug, EntityModel):
            qs = qs.filter(ledger__entity__slug=entity_slug)
        elif isinstance(entity_slug, str):
            qs = qs.filter(ledger__entity__slug__exact=entity_slug)
        if customer:
            qs = qs.filter(itemthroughmodel__jcd_model__work_order__customer__uuid__in=[customer.uuid])
        return qs.values('customer__customer_name',
                'draft_date',
                'amount_due',
                'amount_paid',
                'invoice_number',
                'itemthroughmodel__jcd_model__work_order'
        )
        
        
    def for_work_order_detail(self, entity_slug, work_order=None):
        qs = self.get_queryset().all()
        if isinstance(entity_slug, EntityModel):
            qs = qs.filter(ledger__entity__slug=entity_slug)
        elif isinstance(entity_slug, str):
            qs = qs.filter(ledger__entity__slug__exact=entity_slug)
        if work_order:
            qs = qs.filter(itemthroughmodel__jcd_model__work_order__uuid__in=[work_order.uuid])
        return qs.values('customer__customer_name',
                'draft_date',
                'amount_due',
                'amount_paid',
...
class InvoiceModelAbstract(LedgerWrapperMixIn,
    ...
    cash_account = models.ForeignKey('aix.AccountModel',
                                     on_delete=models.RESTRICT,
                                     verbose_name=_('Cash Account'),
                                     related_name=f'{REL_NAME_PREFIX}_cash_account')
    prepaid_account = models.ForeignKey('aix.AccountModel',
                                        on_delete=models.RESTRICT,
                                        verbose_name=_('Prepaid Account'),
                                        related_name=f'{REL_NAME_PREFIX}_prepaid_account')
    unearned_account = models.ForeignKey('aix.AccountModel',
                                         on_delete=models.RESTRICT,
    ...
    invoice_items = models.ManyToManyField('aix.ItemModel',
    ...
    def get_mark_as_draft_url(self):
        return reverse('aix:invoice-action-mark-as-draft',
                       kwargs={
    ...
    def get_mark_as_review_url(self):
        return reverse('aix:invoice-action-mark-as-review',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_approved_url(self):
        return reverse('aix:invoice-action-mark-as-approved',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_paid_url(self):
        return reverse('aix:invoice-action-mark-as-paid',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_void_url(self):
        return reverse('aix:invoice-action-mark-as-void',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_canceled_url(self):
        return reverse('aix:invoice-action-mark-as-canceled',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_paid_url(self, entity_slug):
        return reverse('aix:invoice-mark-paid',
                       kwargs={
                           'entity_slug': entity_slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_itemthrough_data(self, queryset=None) -> tuple:
        if not queryset:
            ...
            queryset = self.itemthroughmodel_set.select_related(
                'item_model', 'jcd_model', 'invoice_model').all()
