...
from aix.io import IOMixIn
from aix.io.roles import ASSET_CA_CASH, EQUITY_CAPITAL, EQUITY_COMMON_STOCK, EQUITY_PREFERRED_STOCK, \
    validate_roles
from aix.models.accounts import AccountModel, CREDIT
from aix.models.coa import ChartOfAccountModel
from aix.models.coa_default import CHART_OF_ACCOUNTS
from aix.models.mixins import CreateUpdateMixIn, SlugNameMixIn, ContactInfoMixIn
from aix.models.utils import LazyLoader
...
def inventory_adjustment(counted_qs, recorded_qs) -> defaultdict:
    ...
    # counted_map = {counted_qs}
    recorded_map = {}
    if recorded_qs is not None:
        recorded_map = {
...
class EntityModelAbstract(#MP_Node,
    ...
    picture = models.TextField(blank=True, null=True) #models.ImageField(blank=True, null=True)
    ...
    def get_aix_dashboard_url(self, year, month):
        return reverse('aix:entity-dashboard-aix-year',
                       kwargs={
                           'entity_slug': self.slug,
                           'year': year,
                           'month': month,
    ...
    def get_staff_dashboard_url(self):
    ...
    def get_manage_url(self):
        return reverse('aix:entity-update',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_ledgers_url(self):
        return reverse('aix:ledger-list',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_bills_url(self):
        return reverse('aix:bill-list',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_invoices_url(self):
        return reverse('aix:invoice-list',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_banks_url(self):
        return reverse('aix:bank-account-list',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_balance_sheet_url(self):
        return reverse('aix:entity-bs',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_income_statement_url(self):
        return reverse('aix:entity-ic',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_data_import_url(self):
        return reverse('aix:data-import-jobs-list',
                       kwargs={
                           'entity_slug': self.slug
                       })

    def get_accounts_url(self):
        return reverse('aix:account-list',
                       kwargs={
                           'entity_slug': self.slug,
                       })
    ...
    def get_vendors_url(self):
        return reverse('aix:vendor-list',
                       kwargs={
                           'entity_slug': self.slug,
                       })

    def get_delete_url(self):
        return reverse('aix:entity-delete',
                       kwargs={
                           'entity_slug': self.slug
                       })
