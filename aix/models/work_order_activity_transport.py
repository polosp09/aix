...
class WorkOrderActivityTransportModelManager(models.Manager):
...
class WorkOrderActivityTransportModel(CreateUpdateMixIn):
    ...
    entity = models.ForeignKey('aix.EntityModel',
                               ...
                               verbose_name=_('Work Order Transport Entity'))
    ...
    from_location = models.ForeignKey('aix.CustomerLocationModel', 
                                ...
                                related_name='from_location_transport', 
                                on_delete=models.CASCADE,
                                blank=True, null=True)  # Field name made lowercase.
    to_location = models.ForeignKey('aix.CustomerLocationModel', 
                                db_column='ToLocation', 
                                related_name='to_location_transport', 
                                on_delete=models.CASCADE,
                                blank=True, null=True)  # Field name made lowercase.
    driver = models.ForeignKey('aix.EmployeeModel',
                                 on_delete=models.RESTRICT,
                                 verbose_name=_('Driver'))
    vehicle = models.ForeignKey('aix.AssetModel',
                                 on_delete=models.RESTRICT,
                                 verbose_name=_('Vehicle'))
    assistants = models.ManyToManyField('aix.EmployeeModel', 
                                 blank=True, 
                                 related_name='assistants_transport', 
                                 verbose_name=_('Assistants'))

    objects = WorkOrderActivityTransportModelManager()

    class Meta:
        verbose_name = _('Work Order Transport')
        indexes = [
        ...
        unique_together = [
            ('entity', 'activity', 'trip_code', 'driver', 'vehicle')
        ]

    def __str__(self):
        return f'Activity Transport: {self.code} - {self.trip_code} - {self.driver} - {self.vehicle} - {self.activity}`' 
