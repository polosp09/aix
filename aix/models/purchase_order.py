...
from aix.models import EntityModel, ItemThroughModel, LazyLoader, BillModel
...
class PurchaseOrderModelAbstract(CreateUpdateMixIn, MarkdownNotesMixIn):
    ...
    PO_MODULES = [
        (PO_MODULE_DEFAULT, _('Default')),
        (PO_MODULE_FUEL, _('Fuel')),
    ]
    ...
    po_module = models.CharField(max_length=10, choices=PO_MODULES, default=PO_MODULES[0][0])
    ...
    def is_module_default(self) -> bool:
        return self.po_module == self.PO_MODULE_DEFAULT

    def is_module_fuel(self) -> bool:
        return self.po_module == self.PO_MODULE_FUEL

    def can_fuel(self,po_active_module) -> bool:
        return self.PO_MODULE_FUEL == po_active_module
    ...
    def get_mark_as_draft_url(self):
        return reverse('aix:po-action-mark-as-draft',
                       kwargs={
    ...
    def get_mark_as_review_url(self):
        return reverse('aix:po-action-mark-as-review',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })
    ...
    def get_mark_as_approved_url(self):
        return reverse('aix:po-action-mark-as-approved',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })
    ...
    def get_mark_as_canceled_url(self):
        return reverse('aix:po-action-mark-as-canceled',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })
    ...
    def get_mark_as_fulfilled_url(self):
        return reverse('aix:po-action-mark-as-fulfilled',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })
    ...
    def get_mark_as_void_url(self):
        return reverse('aix:po-action-mark-as-void',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })
    ...
    # FPO........................................................
    # DRAFT...
    def mark_as_draft_fuel(self, commit: bool = False, **kwargs):
        if not self.can_draft():
            raise ValidationError(message=f'Fuel Order {self.po_number} cannot be marked as draft.')
        self.po_status = self.PO_STATUS_DRAFT
        self.clean()
        if commit:
            self.save(update_fields=[
    ...
    def get_mark_as_draft_html_id_fuel(self):
        return f'djl-{self.uuid}-po-mark-as-draft-fuel'

    def get_mark_as_draft_url_fuel(self):
        return reverse('aix:po-action-mark-as-draft-fuel',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })

    def get_mark_as_draft_message_fuel(self):
        return _('Do you want to mark Fuel Order %s as Draft?') % self.po_number

    # REVIEW...
    def mark_as_review_fuel(self, date_review: date = None, commit: bool = False, **kwargs):
        if not self.can_review():
            raise ValidationError(message=f'Fuel Order {self.po_number} cannot be marked as in review.')
        itemthrough_qs = self.itemthroughmodel_set.all()
        if not itemthrough_qs.count():
            raise ValidationError(message='Cannot review a FPO without items...')
        if not self.po_amount:
            raise ValidationError(message='FPO amount is zero.')
    ...
    def get_mark_as_review_html_id_fuel(self):
        return f'djl-{self.uuid}-po-mark-as-review-fuel'

    def get_mark_as_review_url_fuel(self):
        return reverse('aix:po-action-mark-as-review-fuel',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })

    def get_mark_as_review_message_fuel(self):
        return _('Do you want to mark Fuel Order %s as In Review?') % self.po_number

    # APPROVED...
    def mark_as_approved_fuel(self, commit: bool = False, date_approved: date = None, **kwargs):
        if not self.can_approve():
            raise ValidationError(message=f'Fuel Order {self.po_number} cannot be marked as approved.')
        self.approved_date = localdate() if not date_approved else date_approved
        self.po_status = self.PO_STATUS_APPROVED
        self.clean()
        if commit:
    ...
    def get_mark_as_approved_html_id_fuel(self):
        return f'djl-{self.uuid}-po-mark-as-approved-fuel'

    def get_mark_as_approved_url_fuel(self):
        return reverse('aix:po-action-mark-as-approved-fuel',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })

    def get_mark_as_approved_message_fuel(self):
        return _('Do you want to mark Fuel Order %s as Approved?') % self.po_number

    # CANCEL...
    def mark_as_canceled_fuel(self, commit: bool = False, date_canceled: date = None, **kwargs):
        if not self.can_cancel():
            raise ValidationError(message=f'Fuel Order {self.po_number} cannot be marked as canceled.')
        self.canceled_date = localdate() if not date_canceled else date_canceled
        self.po_status = self.PO_STATUS_CANCELED
        self.clean()
        if commit:
    ...
    def get_mark_as_canceled_html_id_fuel(self):
        return f'djl-{self.uuid}-po-mark-as-canceled-fuel'

    def get_mark_as_canceled_url_fuel(self):
        return reverse('aix:po-action-mark-as-canceled-fuel',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })

    def get_mark_as_canceled_message_fuel(self):
        return _('Do you want to mark Fuel Order %s as Canceled?') % self.po_number

    # FULFILL...
    def mark_as_fulfilled_fuel(self,
                          date_fulfilled: date = None,
                          po_items: Union[QuerySet, List[ItemThroughModel]] = None,
                          commit=False,
                          **kwargs):
        if not self.can_fulfill():
            raise ValidationError(message=f'Fuel Order {self.po_number} cannot be marked as fulfilled.')
        self.fulfillment_date = localdate() if not date_fulfilled else date_fulfilled
        ...
        all_items_billed = all(bill_models)
        if not all_items_billed:
            raise ValidationError('All items must be billed before FPO can be fulfilled.')

        all_bills_paid = all(b.is_paid() for b in bill_models)
        if not all_bills_paid:
            raise ValidationError('All Bills must be paid before FPO can be fulfilled.')

        all_items_received = all(i.is_received() for i in po_items)
        if not all_items_received:
            raise ValidationError('All items must be received before FPO is fulfilled.')
    ...
    def get_mark_as_fulfilled_html_id_fuel(self):
        return f'djl-{self.uuid}-po-mark-as-fulfilled-fuel'

    def get_mark_as_fulfilled_url_fuel(self):
        return reverse('aix:po-action-mark-as-fulfilled-fuel',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })

    def get_mark_as_fulfilled_message_fuel(self):
        return _('Do you want to mark Fuel Order %s as Fulfilled?') % self.po_number

    # VOID...
    def mark_as_void_fuel(self,
                     entity_slug: str,
                     user_model,
                     void_date: date = None,
                     commit=False,
                     **kwargs):
        if not self.can_void():
            raise ValidationError(message=f'Fuel Order {self.po_number} cannot be marked as void.')
        ...
        if not all(b.is_void() for b in bill_model_qs):
            raise ValidationError('Must void all FPO bills before FPO can be voided.')
    ...
    def get_mark_as_void_html_id_fuel(self):
        return f'djl-{self.uuid}-po-mark-as-void-fuel'

    def get_mark_as_void_url_fuel(self):
        return reverse('aix:po-action-mark-as-void-fuel',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'po_pk': self.uuid
                       })

    def get_mark_as_void_message_fuel(self):
        return _('Do you want to mark Fuel Order %s as Void?') % self.po_number
