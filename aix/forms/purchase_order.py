...
from aix.models import (ItemModel, PurchaseOrderModel, ItemThroughModel, EntityUnitModel)
    ...
    class Meta:
        ...
        widgets = {
            ...
            'po_title': TextInput(attrs={
                'class': MANDO_FORM_INPUT_CLASSES + ' is-large w-100',
...
class BasePurchaseOrderModelUpdateForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'po_status': Select(attrs={'class': MANDO_FORM_INPUT_CLASSES}),
            'vendor': Select(attrs={'class': MANDO_FORM_INPUT_CLASSES}),
...
class PurchaseOrderFuelItemForm(PurchaseOrderItemForm):
    class Meta(PurchaseOrderItemForm):
        fields = [
            'item_model',
            'po_unit_cost',
            'po_quantity',
...
class BasePurchaseOrderItemFormset(BaseModelFormSet):
    ...
    def __init__(self, *args, entity_slug, user_model, po_model: PurchaseOrderModel, **kwargs):
        ...
        for form in self.forms:
            ...
            if self.PO_MODEL.is_module_fuel():
                items_qs = ItemModel.objects.for_po_fuel(
                    entity_slug=self.ENTITY_SLUG,
                    user_model=self.USER_MODEL
                )
            ...
            if not self.PO_MODEL.can_edit_items():
                ...
                if self.PO_MODEL.is_module_fuel():
                    form.fields['jcd_fuel_type'].disabled = True
...
CanEditFuelPurchaseOrderItemFormset = modelformset_factory(
    model=ItemThroughModel,
    form=PurchaseOrderFuelItemForm,
    formset=BasePurchaseOrderItemFormset,
    can_delete=True,
    extra=5
)
...
ReadOnlyFuelPurchaseOrderItemFormset = modelformset_factory(
    model=ItemThroughModel,
    form=PurchaseOrderFuelItemForm,
    formset=BasePurchaseOrderItemFormset,
    can_delete=False,
...
def get_po_fuel_item_formset(po_model: PurchaseOrderModel):
    if po_model.is_draft():
        return CanEditFuelPurchaseOrderItemFormset
    return ReadOnlyFuelPurchaseOrderItemFormset
