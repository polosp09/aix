...
from aix.io.roles import ASSET_CA_CASH, ASSET_CA_PREPAID, LIABILITY_CL_DEFERRED_REVENUE
from aix.models import (ItemModel, AccountModel, BillModel, ItemThroughModel,
                                  VendorModel, EntityUnitModel, WorkOrderJobcardModel)
...
class BaseBillModelUpdateForm(BillModelCreateForm):
    ...
    def save(self, commit=True):
        if commit:
            ...
            if self.BILL_MODEL.can_migrate():
    ...
    class Meta:
        ...
        widgets = {
            'xref': TextInput(attrs={'class': MANDO_FORM_INPUT_CLASSES,
                                     'placeholder': 'External Reference...'}),
            'date': DateInput(attrs={'class': MANDO_FORM_INPUT_CLASSES, 'type': 'date'}),
            'amount_due': TextInput(attrs={'class': MANDO_FORM_INPUT_CLASSES, 'placeholder': '$$$'}),
            'terms': Select(attrs={'class': MANDO_FORM_INPUT_CLASSES}),
            'bill_status': Select(attrs={'class': MANDO_FORM_INPUT_CLASSES}),
            'paid_date': DateInput(
                attrs={
                    'class': MANDO_FORM_INPUT_CLASSES,
            ...
            'progress': TextInput(attrs={'class': MANDO_FORM_INPUT_CLASSES}),
            'accrue': CheckboxInput(attrs={'type': 'checkbox'}),
            'paid': CheckboxInput(attrs={'type': 'checkbox'}),
            'cash_account': Select(attrs={'class': MANDO_FORM_INPUT_CLASSES + ' is-danger'}),
            'prepaid_account': Select(attrs={'class': MANDO_FORM_INPUT_CLASSES + ' is-danger'}),
            'unearned_account': Select(attrs={'class': MANDO_FORM_INPUT_CLASSES + ' is-danger'}),
...
class BaseBillItemFormset(BaseModelFormSet):
    ...
    def __init__(self, *args, entity_slug, bill_model: BillModel, user_model, **kwargs):
        ...
        for form in self.forms:
            ...
            form.fields['jcd_model'].queryset = jcd_qs
            ...
            if instance.po_model_id:
                ...
                form.fields['entity_unit'].disabled = True
            # else:
            #     items_qs = items_qs.filter(item_type__in='F')
            #     form.fields['item_model'].queryset = items_qs
