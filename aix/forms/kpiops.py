...
from aix.models import KpiOpsModel

class KpiOpsForm(ModelForm):
    class Meta:
        model = KpiOpsModel
    ...
    def __init__(self,wo_task,*args,**kwargs):
        super(KpiOpsForm,self).__init__(*args,**kwargs)
    ...
    def get_info(self, email=None, kpiops=None):
        """
        ...
        from_email = email
        subject = f'NEW TASK ACTIVITY CREATED + {kpiops.code} + from aix Portal'
        ...
        msg = f'Created {kpiops} with email {from_email}:'
        msg += f'\n"{subject}"\n\n'
        msg += f'You have created a new task activity number #. {kpiops.code} \n\nDescription\n{description} \n\n\nPlease dont reply this email message\naix@Administrator'

        return subject, msg, cx
    
    def send(self, email=None, kpiops=None):
        subject, msg, cx = self.get_info(email, kpiops)
