...
from aix.exceptions import InvalidDateInputException, TransactionNotInBalanceException
from aix.io import roles
from aix.io.ratios import FinancialRatioManager
from aix.io.roles import RoleManager, GroupManager
from aix.models.utils import LazyLoader
from aix.settings import (MANDO_TRANSACTION_MAX_TOLERANCE,
...
def diff_tx_data(tx_data: list, raise_exception: bool = True):
    ...
    if not is_valid and abs(diff) > MANDO_TRANSACTION_MAX_TOLERANCE:
        if raise_exception:
            raise TransactionNotInBalanceException(
                f'Invalid tx data. vb Credits and debits must match. Currently cr: {CREDITS}, db {DEBITS}.'
                f'Max Tolerance {MANDO_TRANSACTION_MAX_TOLERANCE}'
            )
...
def balance_tx_data(tx_data: list, perform_correction: bool = True) -> bool:
    if tx_data:
        ...
        if not perform_correction and abs(diff) > MANDO_TRANSACTION_MAX_TOLERANCE:
            return False
        ...
        while not is_valid:
            ...
            if len(txs_candidates) > 0:
                ...
                if any([diff > 0 and tx_type_choice == 'debit',
                    ...
                    if IS_TX_MODEL:
                        tx.amount += MANDO_TRANSACTION_CORRECTION
                    else:
                        tx['amount'] += MANDO_TRANSACTION_CORRECTION
...
class IOMixIn:
    ...
    def database_digest(self,
        ...
        if not queryset:
            ...
            # If IO is on unit model....
                ...
                if not entity_slug:
                    raise ValidationError('Calling digest from Branch requires entity_slug')
                txs_qs = TransactionModel.objects.for_unit(
                    user_model=user_model,
    ...
    def digest_invoice_counts(self, invoice_qs, invoice_state=None):
        tot_number = 0
