...
from aix.forms.estimate import (EstimateModelCreateForm, BaseEstimateModelUpdateForm,
                                          #CanEditEstimateItemFormset, ReadOnlyEstimateItemFormset,
                                          DraftEstimateModelUpdateForm)
from aix.models import EntityModel, ItemThroughModel
from aix.models.estimate import EstimateModel
from aix.views import LoginRequiredMixIn


class EstimateModelListView(LoginRequiredMixIn, ArchiveIndexView):
    template_name = 'aix/estimate/estimate_list.html'
...
class EstimateModelCreateView(LoginRequiredMixIn, CreateView):
    ...
    template_name = 'aix/estimate/estimate_create.html'
...
class EstimateModelDetailView(LoginRequiredMixIn, DetailView):
    pk_url_kwarg = 'ce_pk'
    template_name = 'aix/estimate/estimate_detail.html'
...
class EstimateModelUpdateView(LoginRequiredMixIn, UpdateView):
    template_name = 'aix/estimate/estimate_update.html'
    pk_url_kwarg = 'ce_pk'
    ...
    # def get_context_data(self, item_formset: CanEditEstimateItemFormset = None, **kwargs):
    ...
    #         item_formset = CanEditEstimateItemFormset(
    #             entity_slug=self.kwargs['entity_slug'],
    #             user_model=self.request.user,
    ...
    #         )
    #     else:
    #         item_formset = ReadOnlyEstimateItemFormset(
    #             entity_slug=self.kwargs['entity_slug'],
    #             user_model=self.request.user,
    #             customer_job_model=cj_model,
    #             queryset=item_through_qs
    ...
    def post(self, request, entity_slug, ce_pk, *args, **kwargs):
        ...
        if self.action_update_items:
            item_formset: CanEditEstimateItemFormset = CanEditEstimateItemFormset(request.POST,
                                                                                            user_model=self.request.user,
            ...
            if item_formset.is_valid():
                if item_formset.has_changed():
                    ...
                    return HttpResponseRedirect(reverse('aix:customer-estimate-update',
                                                        kwargs={
                                                            'entity_slug': entity_slug,
                                                            'ce_pk': ce_pk
                                                        }))
...
class BaseEstimateActionView(LoginRequiredMixIn, RedirectView, SingleObjectMixin):
    ...
    def get_redirect_url(self, entity_slug, ce_pk, *args, **kwargs):
        return reverse('aix:customer-estimate-update',
                       kwargs={
                           'entity_slug': entity_slug,
                           'ce_pk': ce_pk
                       })
