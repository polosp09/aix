...
from aix.models import EntityModel, BillModel, VendorModel
from aix.settings import MANDO_LOGIN_URL
from aix.tests.base import DjangoLedgerBaseTest
from aix.urls.bill import urlpatterns as bill_urls
...
class BillModelTests(DjangoLedgerBaseTest):
    ...
    def test_bill_list(self):
        ...
        bill_list_url = reverse('aix:bill-list',
                                kwargs={
        ...
        for bill_model in bill_model_qs:
            ...
            bill_detail_url = reverse('aix:bill-detail',
                                      kwargs={
                                          'entity_slug': entity_model.slug,
                                          'bill_pk': bill_model.uuid
                                      })
            bill_update_url = reverse('aix:bill-update',
                                      kwargs={
                                          'entity_slug': entity_model.slug,
                                          'bill_pk': bill_model.uuid
                                      })
            bill_delete_url = reverse('aix:bill-delete',
                                      kwargs={
                                          'entity_slug': entity_model.slug,
                                          'bill_pk': bill_model.uuid
                                      })
            mark_as_paid_url = reverse('aix:bill-mark-paid',
                                       kwargs={
                                           'entity_slug': entity_model.slug,
                                           'bill_pk': bill_model.uuid
                                       })
    ...
    def test_bill_create(self):
        ...
        bill_create_url = reverse('aix:bill-create',
                                  kwargs={
                                      'entity_slug': entity_model.slug
                                  })
        ...
        bill_data['prepaid_account'] = cash_account.uuid
        create_response = self.CLIENT.post(bill_create_url, data=bill_data, follow=True)
        self.assertTrue(create_response.resolver_match.view_name, 'aix:bill-detail')
